...
RingBufReadOne(tRingBufObject *psRingBuf)
{
    ...
    //
    assert_param(RingBufUsed(psRingBuf) != 0);
...
RingBufRead(tRingBufObject *psRingBuf, uint8_t *pui8Data, uint32_t ui32Length)
{
    ...
    //
    assert_param(psRingBuf != NULL);
    assert_param(pui8Data != NULL);
    assert_param(ui32Length != 0);

    //
    // Verify that data is available in the buffer.
    //
    assert_param(ui32Length <= RingBufUsed(psRingBuf));
                    ...
                    uint32_t ui32NumBytes)
{
    ...
    //
    assert_param(ui32NumBytes <= psRingBuf->ui32Size);
...
RingBufWriteOne(tRingBufObject *psRingBuf, uint8_t ui8Data)
{
    ...
    // Verify that space is available in the buffer.
    //
    assert_param(RingBufFree(psRingBuf) != 0);
             ...
             uint32_t ui32Length)
{
    ...
    // Verify that space is available in the buffer.
    //
    assert_param(ui32Length <= RingBufFree(psRingBuf));
            ...
            uint32_t ui32Size)
{
    ...
    //
    assert_param(psRingBuf != NULL);
    assert_param(pui8Buf != NULL);
    assert_param(ui32Size != 0);
